...
and five repeats. The six clinical inputs are fixed throughout this analysis;
clinical-variable selection is not evaluated. Radiomics-containing models
start each outer evaluation from the ICC-qualified candidate pool, not the
...
The three configured thresholds are locked values reported for the final
models. `threshold_audit.csv` separately reports the Youden thresholds
calculated from the fitted models' training probabilities; disagreement means
the configured values were not regenerated by this run. This public
implementation and its unit tests do not establish that manuscript AUCs or
feature lists can be reproduced without the original data and extraction
configuration. The extraction settings enable LBP3D, consistent with the
reported final feature names; raw column names and feature counts still need
comparison against the study's original extraction outputs before claiming
numerical reproduction.
After full-data selection, `s10_feature_name_audit.csv` compares each reported
...
```text
patient_id,cohort,outcome,culture_positive,targeted_molecular_positive,mngs_positive,WBC,Hb,CRP,GLB,VAS,spinal_tenderness,original_...,wavelet_...
P001,training,1,1,0,0,8.4,121,32.0,35.2,6,1,...,...
```
...
The strict-reference sensitivity analysis retains cases with culture, targeted
PCR/NAAT, and/or mNGS positive evidence, including mNGS-only cases. It excludes
histopathology-only cases and applies the locked models without retraining or
threshold re-optimisation. The three positive-evidence columns must each be
explicitly recorded as 1/0 or true/false; free-text `reference_standard` is
not used to infer positivity. Unknown evidence must be resolved before running
this analysis. `strict_reference_counts.csv` reports cohort-specific inclusions
and exclusions for comparison with the manuscript's 18/9/6 exclusions.
The analysis stops if the explicit evidence fields are missing or unknown;
the supplied template demonstrates their expected coding. The cohort counts
are measured from the input data and are not preset to 18/9/6.
